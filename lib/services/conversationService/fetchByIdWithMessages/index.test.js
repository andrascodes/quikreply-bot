...
test(`ConversationService#fetchByIdWithMessages`, nest => {

  const { db, fetchByIdWithMessages } = setup()
    
  nest.test(`ConversationService#fetchByIdWithMessages: should return empty array if 'id' is not passed`, assert => {
    const id = undefined
    assert.deepEqual(fetchByIdWithMessages(id), [])
    assert.end()
  })

  nest.test(`ConversationService#fetchByIdWithMessages: should return null if a Conversation with the passed id is not found `, assert => {
    const id = 44
  ...
  nest.test(`ConversationService#fetchByIdWithMessages: should rethrow the Error from inside the method `, assert => {
    const id = 'asdasds'
    db.sequelize.sync({ force: true })
    .then(() => db.Conversation.create({ id: 1 }))
    .then(() => fetchByIdWithMessages(id))
  ...
  nest.test(`ConversationService#fetchByIdWithMessages: should return the Conversation object with the passed id`, assert => {
    const id = 44
    db.sequelize.sync({ force: true })
  ...
  nest.test(`ConversationService#fetchByIdWithMessages: should return the Messages belonging to the Conversation`, assert => {
    const id = 44
    const messageId = 1
    db.sequelize.sync({ force: true })
    .then(() => db.Conversation.create({ id }))
  ...
  nest.test(`ConversationService#fetchByIdWithMessages: should return the necessary fields of the Conversation and the Messages`, assert => {
    const id = 44
    const messageId = 1
    db.sequelize.sync({ force: true })
    .then(() => db.Conversation.create({ id }))

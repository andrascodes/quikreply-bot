...
class BootBot extends EventEmitter {
  ...
  sendListTemplate(recipientId, elements, options) {
    const payload = {
      template_type: 'list',
      top_element_style: options.top_element_style,
      elements,
  ...
  sendTemplate(recipientId, payload, options) {
    ...
    if(options && options.quickReplies) {
      message.quick_replies = this._formatQuickReplies(options.quickReplies)
    }
  ...
  _formatQuickReplies(quickReplies) {
    return quickReplies && quickReplies.map((reply) => {
      ...
      } else if (reply && reply.content_type === 'location') {
        return {
